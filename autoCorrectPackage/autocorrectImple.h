...
// 	}
	...
	for(std::string dict_word;dictionary >> dict_word;)
		if(wrong_word.front() > dict_word.front())
			...
			if(uncommon_letter_count == 1) {
				ans.push_back(dict_word);
				//std::cout << dict_word << ", ";
				found = true;
				++word_count;
			}
...
bool AutoCorrect::checkMissingLetters(){//const unsigned int &MISSING) const noexcept {
	...
	for(std::string dict_word;dictionary >> dict_word;)
		if(wrong_word.front() > dict_word.front())		continue;
		...
		else if(wrong_word.length() + 1 == dict_word.length()) {
			...
			if(wrong_letter == wrong_word.cend() && missing_letter_count == 1) {
				//std::cout << dict_word << ", ";
				ans.push_back(dict_word);
				found = true;
				++word_count;
...
bool AutoCorrect::checkMissingAndExtraLetters(){//const unsigned int &MISSING, const unsigned int &EXTRA) const noexcept {
	...
	for(std::string dict_word;dictionary >> dict_word;)
		if(wrong_word.front() > dict_word.front())
			...
			break;
		else if(wrong_word.length() + 1 == dict_word.length() + 1) {
			...
			if(missing_letter_count == 1 && extra_letter_count == 1) {
				//std::cout << dict_word << ", ";
				ans.push_back(dict_word);
				found = true;
				++word_count;
